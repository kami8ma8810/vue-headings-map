...
export function parseVueFile(filePath: string): HeadingNode[] {
  try {
    ...
    if (templateMatch && templateMatch.index !== undefined) {
      ...
      // まずフラットな配列として見出しを抽出
      const templateHeadings = extractHeadingsFromTemplate(template, lineOffset);
      headings = [...headings, ...templateHeadings];
    }
 ...
 * これは単一のファイル内の見出し構造を検証します
 */
function validateHeadingStructure(headings: HeadingNode[]): void {
  ...
  const h3PlusHeadings = headings.filter(h => h.level >= 3);
  
  console.log(`Found ${h1Headings.length} h1 headings, ${h2Headings.length} h2 headings, and ${h3PlusHeadings.length} h3+ headings`);
  ...
  if (firstH1) {
    ...
    if (firstH1Index > 0) {
      ...
      // それらの見出しに警告を追加
      headingsBeforeH1.forEach(heading => {
        if (heading.level >= 2) {
          heading.hasWarning = true;
      ...
      // 元の配列内の該当する要素も更新するために、元の配列を直接操作
      headings.forEach(h => {
        const beforeH1Heading = headingsBeforeH1.find(bh => 
          bh.level === h.level && bh.line === h.line && bh.content === h.content);
        if (beforeH1Heading && beforeH1Heading.hasWarning) {
          h.hasWarning = true;
          h.warningMessage = beforeH1Heading.warningMessage;
        }
      });
    }
  }
  
  // ステップ2: 逆順の見出しレベルとレベルスキップをチェック
  let lastLevel = 0;
  ...
  // 検証結果を確認し、警告がある見出しをハイライト表示
  console.log("\n==== WARNINGS SUMMARY ====");
  const warningHeadings = headings.filter(h => h.hasWarning);
  if (warningHeadings.length > 0) {
    warningHeadings.forEach(h => {
      console.log(`WARNING: h${h.level} "${h.content}" - ${h.warningMessage}`);
    });
  } else {
